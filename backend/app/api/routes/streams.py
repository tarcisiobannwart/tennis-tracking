...
def _format_stream_response(stream: dict) -> dict:
    """Format a stream document to match the response schema"""
    return {
        "id": stream["_id"],
...
async def get_match_streams(
    ...
    streams = await stream_service.get_streams_for_match(match_id)
    active_count = sum(1 for s in streams if s["status"] == "live")
...
async def update_stream(
    ...
    if request.is_recording is not None:
        stream = await stream_service.set_recording(stream_id, request.is_recording)
...
async def end_stream(
    ...
    if stream["status"] == "live":
        await stream_service.end_stream(stream_id)
...
async def toggle_recording(
    ...
    updated = await stream_service.set_recording(stream_id, is_recording)
...
async def sync_stream_status(
    ...
    stream = await stream_service.sync_stream_status(stream_id)
    if not stream:
        raise HTTPException(status_code=404, detail="Stream not found")
