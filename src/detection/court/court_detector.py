"""
Court Detector - Sistema de Detecção e Rastreamento de Quadra de Tênis

Este módulo implementa um sistema robusto para detectar e rastrear as linhas da quadra
de tênis em vídeos usando técnicas de visão computacional:

- Detecção de linhas usando transformada de Hough
- Homografia para correção de perspectiva
- Filtros morfológicos para realce de características
- Rastreamento temporal para estabilidade

Algoritmo Principal:
1. Binarização da imagem para destacar linhas brancas
2. Filtragem de pixels baseada na estrutura da quadra
3. Detecção de linhas horizontais e verticais
4. Correspondência com modelo de referência da quadra
5. Cálculo de transformação homográfica
6. Rastreamento temporal nas próximas frames
...
class CourtDetector:
    """
    Classe para detecção e rastreamento de quadra de tênis em vídeos.

    Esta classe implementa algoritmos de visão computacional para:
    - Detectar automaticamente as linhas da quadra de tênis
    - Rastrear a quadra ao longo do tempo
    - Corrigir transformações de perspectiva
    - Mapear coordenadas entre quadra real e imagem

    Attributes:
        colour_threshold (int): Limite para binarização (padrão: 200)
        dist_tau (int): Distância para filtro morfológico (padrão: 3)
        intensity_threshold (int): Limite de intensidade para filtros (padrão: 40)
        court_reference (CourtReference): Modelo de referência da quadra
        court_warp_matrix (list): Matrizes de transformação para cada frame
        game_warp_matrix (list): Matrizes inversas de transformação
        court_score (float): Pontuação de confiança da detecção
        success_flag (bool): Flag indicando sucesso na detecção
        best_conf (int): Melhor configuração encontrada

    Methods:
        detect: Detectar quadra em um frame
        track_court: Rastrear quadra após detecção inicial
        add_court_overlay: Adicionar sobreposição da quadra
        find_lines_location: Encontrar posições das linhas importantes
        get_warped_court: Obter quadra transformada
    """
    def __init__(self, verbose=0):
        """
        Inicializa o detector de quadra com parâmetros padrão.

        Args:
            verbose (int): Nível de verbosidade para debug (0=silencioso, 1=verbose)
        """
        # Configurações de debug
        self.verbose = verbose

        # Parâmetros de detecção
        self.colour_threshold = 200  # Limiar para binarização de pixels brancos
        self.dist_tau = 3  # Distância para filtro morfológico
        self.intensity_threshold = 40  # Limiar de intensidade para filtros

        # Referência da quadra padrão
        ...
        # Estado atual
        self.frame = None  # Frame atual sendo processado
        self.gray = None  # Versão em escala de cinza do frame

        # Matrizes de transformação
        self.court_warp_matrix = []  # Transformações quadra -> frame
        self.game_warp_matrix = []  # Transformações frame -> quadra

        # Métricas de qualidade
        self.court_score = 0  # Pontuação de confiança da detecção

        # Linhas da quadra detectadas (coordenadas no frame)
        self.baseline_top = None  # Linha de base superior
        self.baseline_bottom = None  # Linha de base inferior
        self.net = None  # Rede
        self.left_court_line = None  # Linha lateral esquerda
        self.right_court_line = None  # Linha lateral direita
        self.left_inner_line = None  # Linha interna esquerda
        self.right_inner_line = None  # Linha interna direita
        self.middle_line = None  # Linha central
        self.top_inner_line = None  # Linha interna superior
        self.bottom_inner_line = None  # Linha interna inferior

        # Controle de sucesso na detecção
        self.success_flag = False
        self.success_accuracy = 80  # Precisão mínima para sucesso
        self.success_score = 1000  # Pontuação mínima para sucesso

        # Estado do rastreamento
        self.best_conf = None  # Melhor configuração encontrada
        self.frame_points = None  # Pontos da quadra no frame atual
        self.dist = 5  # Distância de busca para rastreamento

    def detect(self, frame, verbose=0):
        """
        Detecta a quadra de tênis em um frame de vídeo.

        Este método implementa o pipeline completo de detecção:
        1. Binarização da imagem para destacar linhas brancas
        2. Filtragem morfológica baseada na estrutura da quadra
        3. Detecção de linhas usando transformada de Hough
        4. Correspondência com configurações conhecidas da quadra
        5. Cálculo da melhor transformação homográfica

        Args:
            frame (numpy.ndarray): Frame de entrada em formato BGR
            verbose (int): Nível de verbosidade (0=silencioso, 1=debug)

        Returns:
            numpy.ndarray: Array com coordenadas das linhas importantes da quadra
                          no formato [x1, y1, x2, y2, ...] para cada linha

        Raises:
            ValueError: Se o frame estiver vazio ou em formato inválido

        Example:
            >>> detector = CourtDetector()
            >>> lines = detector.detect(frame)
            >>> print(f"Detectadas {len(lines)//4} linhas da quadra")
        """
        self.verbose = verbose
        self.frame = frame
        self.v_height, self.v_width = frame.shape[:2]

        # Obter imagem binária do frame
        self.gray = self._threshold(frame)

        # Filtrar pixels usando a estrutura conhecida da quadra
        ...
        # Encontrar transformação da quadra de referência para quadra do frame
        ...
        # Encontrar localização das linhas importantes no frame
        return self.find_lines_location()

    def _threshold(self, frame):
        """
        Binarização simples para destacar pixels brancos (linhas da quadra).

        Converte a imagem para escala de cinza e aplica limiarização binária
        para destacar as linhas brancas da quadra contra o fundo mais escuro.

        Args:
            frame (numpy.ndarray): Frame de entrada em formato BGR

        Returns:
            numpy.ndarray: Imagem binária onde pixels brancos representam
                          possíveis linhas da quadra
        """
    ...
    def _filter_pixels(self, gray):
        """
        Filtra pixels usando a estrutura conhecida das linhas da quadra.

        Este filtro morfológico mantém apenas pixels que têm características
        típicas de linhas da quadra:
        - Pixels brancos com contraste alto em relação aos vizinhos
        - Estrutura linear (horizontal ou vertical)
        - Intensidade suficiente para ser considerada linha

        O algoritmo verifica se cada pixel branco tem diferença de intensidade
        significativa em relação aos pixels vizinhos em ambas as direções
        (horizontal e vertical).

        Args:
            gray (numpy.ndarray): Imagem binária de entrada

        Returns:
            numpy.ndarray: Imagem filtrada com apenas pixels que provavelmente
                          pertencem a linhas da quadra
        """
        for i in range(self.dist_tau, len(gray) - self.dist_tau):
            for j in range(self.dist_tau, len(gray[0]) - self.dist_tau):
                # Pular pixels já marcados como fundo (pretos)
                if gray[i, j] == 0:
                    continue

                # Verificar contraste vertical (acima e abaixo)
                vertical_contrast = (
                    gray[i, j] - gray[i + self.dist_tau, j] > self.intensity_threshold and
                    gray[i, j] - gray[i - self.dist_tau, j] > self.intensity_threshold
                )

                # Verificar contraste horizontal (esquerda e direita)
                horizontal_contrast = (
                    gray[i, j] - gray[i, j + self.dist_tau] > self.intensity_threshold and
                    gray[i, j] - gray[i, j - self.dist_tau] > self.intensity_threshold
                )

                # Manter pixel apenas se tiver contraste em pelo menos uma direção
                if not (vertical_contrast or horizontal_contrast):
                    gray[i, j] = 0
