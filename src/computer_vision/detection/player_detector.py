"""
Detection Model - Sistema de Detecção e Rastreamento de Jogadores

Este módulo implementa um sistema de detecção de jogadores de tênis usando:
- Faster R-CNN ResNet50 pré-treinado para detecção de pessoas
- Algoritmo SORT para rastreamento de múltiplos objetos
- Filtros baseados na geometria da quadra para distinguir jogadores de espectadores
- Rastreamento temporal para manter IDs consistentes

Pipeline de Detecção:
1. Detecção inicial usando região de interesse (ROI) baseada na quadra
2. Filtragem de detecções usando máscaras da quadra
3. Seleção do maior bounding box como jogador principal
4. Rastreamento temporal usando localização prévia
5. Associação de detecções para identificar segundo jogador
...
class DetectionModel:
    """
    Classe para detecção e rastreamento de jogadores de tênis em vídeos.

    Utiliza Faster R-CNN pré-treinado combinado com algoritmos de rastreamento
    e filtros baseados na geometria da quadra para detectar e rastrear jogadores
    de forma robusta, distinguindo-os de espectadores e outras pessoas.

    Attributes:
        detection_model: Modelo Faster R-CNN ResNet50 pré-treinado
        dtype: Tipo de tensor (CPU/GPU)
        PERSON_LABEL (int): ID da classe 'pessoa' no COCO dataset (1)
        PERSON_SCORE_MIN (float): Confiança mínima para detecção inicial (0.85)
        PERSON_SECONDARY_SCORE (float): Confiança para rastreamento (0.3)
        player_1_boxes (list): Histórico de bounding boxes do jogador 1
        player_2_boxes (list): Histórico de bounding boxes do jogador 2
        persons_boxes (dict): Detecções de pessoas por frame
        mot_tracker (Sort): Rastreador de múltiplos objetos

    Methods:
        detect_player_1: Detectar jogador principal (mais próximo da câmera)
        detect_top_persons: Detectar pessoas na área superior da quadra
        find_player_2_box: Identificar segundo jogador baseado em movimento
        _detect: Executar detecção usando Faster R-CNN
    """
    def __init__(self, dtype=torch.FloatTensor):
        """
        Inicializa o modelo de detecção com Faster R-CNN pré-treinado.

        Args:
            dtype (torch.dtype): Tipo de tensor para processamento (CPU/GPU)
        """
        # Modelo de detecção Faster R-CNN ResNet50 com FPN
        self.detection_model = torchvision.models.detection.fasterrcnn_resnet50_fpn(pretrained=True)
        self.detection_model.type(dtype)  # Move modelo para GPU se disponível
        self.detection_model.eval()  # Modo de inferência
        self.dtype = dtype

        # IDs das classes no dataset COCO
        self.PERSON_LABEL = 1  # Classe 'pessoa'
        self.RACKET_LABEL = 43  # Classe 'raquete de tênis'
        self.BALL_LABEL = 37  # Classe 'bola de esporte'

        # Limiares de confiança para detecções
        self.PERSON_SCORE_MIN = 0.85  # Confiança mínima para detecção inicial
        self.PERSON_SECONDARY_SCORE = 0.3  # Confiança reduzida para rastreamento
        self.RACKET_SCORE_MIN = 0.6  # Confiança mínima para raquetes
        self.BALL_SCORE_MIN = 0.6  # Confiança mínima para bolas

        # Propriedades do vídeo
        self.v_width = 0  # Largura do frame
        self.v_height = 0  # Altura do frame

        # Rastreamento de jogadores
        self.player_1_boxes = []  # Histórico do jogador 1 (inferior)
        self.player_2_boxes = []  # Histórico do jogador 2 (superior)

        # Detecções de todas as pessoas por frame
        self.persons_boxes = {}  # {frame_id: [bounding_boxes]}
        self.persons_dists = {}  # {frame_id: [distâncias_da_quadra]}
        self.persons_first_appearance = {}  # {person_id: frame_primeira_aparição}

        # Controle de estado
        self.counter = 0  # Contador de frames processados
        self.num_of_misses = 0  # Número de frames sem detecção
        self.last_frame = None  # Frame anterior
        self.current_frame = None  # Frame atual
        self.next_frame = None  # Próximo frame

        # Parâmetros de rastreamento
        self.movement_threshold = 200  # Limiar de movimento para filtrar jogadores

        # Rastreador SORT para múltiplos objetos
        self.mot_tracker = Sort(
            max_age=10,  # Máximo de frames sem detecção antes de remover track
            min_hits=3,  # Mínimo de detecções consecutivas para confirmar track
            iou_threshold=0.05  # Limiar de IoU para associação
        )

    def detect_player_1(self, image, court_detector):
        """
        Detecta o jogador principal (jogador inferior/mais próximo da câmera).

        Este método implementa duas estratégias:
        1. Detecção inicial: Usa região de interesse baseada na quadra
        2. Rastreamento: Usa posição anterior para definir ROI reduzida

        A detecção inicial utiliza máscara da metade inferior da quadra para
        filtrar espectadores e focar apenas em jogadores ativos.

        Args:
            image (numpy.ndarray): Frame atual do vídeo
            court_detector (CourtDetector): Detector de quadra para ROI

        Returns:
            numpy.ndarray or None: Bounding box do jogador detectado ou None
                                  se nenhum jogador foi encontrado

        Example:
            >>> detection_model = DetectionModel()
            >>> box = detection_model.detect_player_1(frame, court_detector)
            >>> if box is not None:
            ...     x1, y1, x2, y2 = box
